...
class Qwen2Attention(nn.Module):
    ...
    def __init__(self, config: Qwen2Config, prompt_config, layer_idx: Optional[int] = None):
        super().__init__()
        self.config = config
        ...
        # CL additions: per-task LoRA and Gaussian distribution tracking
        ...
        # Note: v uses num_key_value_heads (GQA), not num_heads
    ...
    def calculate_distances(self, features, distributions, distance_type='L2', temperature=1.0):
        with torch.no_grad():
            ...
            for gaussian in distributions:
                ...
                var = gaussian.var
                if distance_type == 'L2':
                    dist_val = torch.norm(features - mean, dim=1)
                elif distance_type == 'Gaussian':
                    var = var + 1e-6
                    log_prob = -0.5 * (torch.log(2 * torch.pi * var) + ((features - mean) ** 2) / var)
                    dist_val = log_prob.sum(dim=1)
                elif distance_type == 'Attention':
                    dim = features.size(-1)
                    ty = features.dtype
                    dist_val = torch.matmul(features, mean.type(ty)) / torch.sqrt(torch.tensor(dim))
                elif distance_type == 'Cosine':
                    features_norm = F.normalize(features, dim=1)
                    mean_norm = F.normalize(mean.reshape(1, -1), dim=1)
                    dist_val = torch.matmul(features_norm, mean_norm.t()).reshape(-1)
                else:
    ...
    def top_p_weights(self, key_weights, top_p, norm=True):
        ...
        key_weights = key_weights.squeeze(-1)
        sorted_probs, sorted_indices = torch.sort(key_weights, descending=True, dim=1)
        cumulative_probs = torch.cumsum(sorted_probs, dim=1)
        exceeds_p = cumulative_probs >= top_p
        if top_p < 1.0:
            cutoff_indices = exceeds_p.float().argmax(dim=1)
        ...
        range_tensor = torch.arange(K, device=key_weights.device).unsqueeze(0).expand(B, K)
        cutoff_indices = cutoff_indices.unsqueeze(1)
        mask = range_tensor <= cutoff_indices
        filtered_sorted_probs = sorted_probs * mask.float()
        sum_probs = filtered_sorted_probs.sum(dim=1, keepdim=True).clamp(min=1e-10)
        if norm:
            normalized_sorted_probs = filtered_sorted_probs / sum_probs
        else:
            normalized_sorted_probs = filtered_sorted_probs
        normalized_sorted_probs = normalized_sorted_probs.to(key_weights.dtype)
        top_p_w = torch.zeros_like(key_weights)
        top_p_w.scatter_(1, sorted_indices, normalized_sorted_probs)
        return top_p_w.unsqueeze(-1)
                    ...
                    for t in range(len(all_gpu_up_v_list)):
                        if t != local_rank:
                            for row in all_gpu_up_v_list[t]:
                                self.distribution_v.update(row.to(f'cuda:{local_rank}'))
...
class Qwen2DecoderLayer(nn.Module):
    def __init__(self, config: Qwen2Config, prompt_config, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size
        ...
        self.self_attn = QWEN2_ATTENTION_CLASSES[config._attn_implementation](config, prompt_config, layer_idx)
...
class Qwen2Model(Qwen2PreTrainedModel):
    ...
    def __init__(self, config: Qwen2Config, prompt_config):
        ...
        self.layers = nn.ModuleList(
            [Qwen2DecoderLayer(config, prompt_config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
