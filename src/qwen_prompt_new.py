...
"""PyTorch Qwen model with project-specific continual-learning logic."""
import math
...
try:
    ...
    try:
        ...
        raise ImportError(
            "Qwen2Config is not available. Install transformers>=4.37.0 for Qwen support."
        ) from exc

# Reuse the proven project implementation while binding to Qwen2 config.
...
class LlamaAttention(nn.Module):
    ...
    def __init__(self, config: LlamaConfig, prompt_config):
        ...
        if self.num_heads % self.num_key_value_heads != 0:
            raise ValueError(
                f"num_attention_heads must be divisible by num_key_value_heads (got {self.num_heads} and {self.num_key_value_heads})"
            )
        ...
        self.lora_v = LoRALayer(self.hidden_size, self.num_key_value_heads * self.head_dim, r=prompt_config["lora_r"], lora_alpha=prompt_config["lora_alpha"], lora_dropout=prompt_config["lora_dropout"])
        ...
        if prompt_config["previous_lora_path"] is not None:
            ...
            with torch.no_grad():
                ...
                for i in range(len(previous_lora_list)):
                    layer = LoRALayer(self.hidden_size, self.num_key_value_heads * self.head_dim, r=prompt_config["lora_r"], lora_alpha=prompt_config["lora_alpha"], lora_dropout=prompt_config["lora_dropout"])
    ...
    def agg_lora_states(self, hidden_states, lora_layer, pre_lora_layer, key_attention_weights, output_dim):
        ...
        if pre_lora_layer is not None and num_task > 1:
            ...
            concat_q = torch.cat([cur_lora_states, pre_lora_states], dim=0).transpose(0, 1).reshape(bsz, -1, hidden_states.shape[1] * output_dim)

            agg_lora_states = torch.matmul(key_attention_weights.transpose(1, 2), concat_q).squeeze()

        else:
            cur_lora_states = lora_layer(hidden_states).unsqueeze(0).transpose(0, 1).reshape(bsz, -1, hidden_states.shape[1] * output_dim)
            agg_lora_states = torch.matmul(key_attention_weights.transpose(1, 2), cur_lora_states).squeeze()

        return agg_lora_states.reshape(bsz, -1, output_dim)
    ...
    def forward(
        ...
        if self.key_attention_weights_q is not None:
            query_states = (self.q_proj(hidden_states)+self.agg_lora_states(hidden_states, self.lora_q, self.previous_lora_weights_q, self.key_attention_weights_q, self.num_heads * self.head_dim)).view(bsz, q_len, self.num_heads, self.head_dim).transpose(1, 2)
        else:
        ...
        if self.key_attention_weights_v is not None:
            value_states = (self.v_proj(hidden_states)+self.agg_lora_states(hidden_states, self.lora_v, self.previous_lora_weights_v, self.key_attention_weights_v, self.num_key_value_heads * self.head_dim)).view(bsz, q_len, self.num_key_value_heads, self.head_dim).transpose(1, 2)
        else:
            value_states = (self.v_proj(hidden_states)+self.lora_v(hidden_states)).view(bsz, q_len, self.num_key_value_heads, self.head_dim).transpose(1, 2)
...
class LlamaFlashAttention2(LlamaAttention):
    ...
    def __init__(self, config: LlamaConfig, prompt_config):
        if not _HAS_FLASH_ATTN:
            raise ImportError(
                "flash-attn is not installed but flash attention was requested. "
                "Install flash-attn or set --flash_attention False."
            )
    ...
    def forward(
        ...
        if self.key_attention_weights_q is not None:
            query_states = self.q_proj(hidden_states)+self.agg_lora_states(hidden_states, self.lora_q, self.previous_lora_weights_q, self.key_attention_weights_q, self.num_heads * self.head_dim)
        else:
        ...
        if self.key_attention_weights_v is not None:
            value_states = self.v_proj(hidden_states)+self.agg_lora_states(hidden_states, self.lora_v, self.previous_lora_weights_v, self.key_attention_weights_v, self.num_key_value_heads * self.head_dim)
        else:
...
# Qwen-facing exported classes for the training script.
...
class QwenForSequenceClassification(LlamaForSequenceClassification):
    config_class = Qwen2Config


# Compatibility aliases with upstream naming.
Qwen2Model = QwenModel
