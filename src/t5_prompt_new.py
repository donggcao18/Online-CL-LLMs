...
class T5Attention(nn.Module):
    def __init__(self, config: T5Config, prompt_config, has_relative_attention_bias=False):
        super().__init__()
        ...
        # self.q = lora.Linear(self.d_model, self.inner_dim, r=4, bias=False)
        self.lora_q = LoRALayer(self.d_model, self.inner_dim, r=prompt_config["lora_r"], lora_alpha=prompt_config["lora_alpha"], lora_dropout=prompt_config["lora_dropout"])
        self.k = nn.Linear(self.d_model, self.inner_dim, bias=False)
        # self.v = lora.Linear(self.d_model, self.inner_dim, r=4, bias=False)
        self.v = nn.Linear(self.d_model, self.inner_dim, bias=False)
        self.lora_v = LoRALayer(self.d_model, self.inner_dim, r=prompt_config["lora_r"], lora_alpha=prompt_config["lora_alpha"], lora_dropout=prompt_config["lora_dropout"])
        self.o = nn.Linear(self.inner_dim, self.d_model, bias=False)

        self.distribution_q = None
        self.distribution_v = None
        ...
        if prompt_config["previous_lora_path"] is not None:
            with torch.no_grad():
                ...
                for i in range(prompt_config["task_id"]):
                    layer = LoRALayer(self.d_model, self.inner_dim, r=prompt_config["lora_r"], lora_alpha=prompt_config["lora_alpha"], lora_dropout=prompt_config["lora_dropout"])
        ...
        if (not self.is_decoder) and (prompt_config["previous_lora_distribution_path"] is not None):
            with torch.no_grad():
    ...
    def forward(
        ...
        def agg_lora_states(hidden_states, 
            ...
            if pre_lora_layer is not None:
                ...
                concat_q = torch.cat([cur_lora_states, pre_lora_states], dim=0).transpose(0, 1).reshape(batch_size, -1, hidden_states.shape[1]*self.d_model)
                agg_lora_states = torch.matmul(key_attention_weights.transpose(1, 2), concat_q).squeeze()
            else:
                cur_lora_states = lora_layer(hidden_states).unsqueeze(0).transpose(0, 1).reshape(batch_size, -1, hidden_states.shape[1]*self.d_model)
        ...
        def calculate_distances(features, 
            ...
            with torch.no_grad():
                ...
                if distance_type == 'Gaussian':
                    ...
                    dis_weights = torch.stack(distances, dim=1)  # [B, num_distributions]
                    dis_weights = F.softmax(-dis_weights / temperature, dim=1)
                    # dis_weights = dis_weights / temperature
                    # dis_weights = torch.exp(-dis_weights)
                    # dis_weights = dis_weights / dis_weights.sum(dim=1, keepdim=True)
                else:
                    raise ValueError(f"Unsupported distance type: {distance_type}")
        ...
        def project(hidden_states, 
            ...
            if key_value_states is None:
                ...
                if lora_layer is not None:
                    if key_attention_weights_v is not None:
                        lora_states = agg_lora_states(key_value_states, lora_layer, self.previous_lora_weights_v, key_attention_weights_v)
                    else:
                        lora_states = lora_layer(key_value_states)
                    hidden_states = shape(proj_layer(key_value_states) + lora_states)
                else:
        ...
        def top_k_weights(key_weights,top_k):
        ...
        def top_p_weights(key_weights, top_p, norm=True):
            """
        ...
        if (self.previous_lora_weights_q is not None) and (not self.is_decoder) and(hidden_states.shape[1]>1):
            if past_key_attention_weights_q is not None:
                ...
                with torch.no_grad():
                    key_q=None
                    for each_q,each_ids_w in zip(self.q(hidden_states),input_ids):
                        each_q=each_q[:len(each_ids_w)-(each_ids_w==0).long().sum()]
                        each_q=torch.mean(each_q,dim=0)
                        if key_q is None:
                    ...
                    if (self.training) and (self.train_key_weight_top_p > 0):
                        self.key_attention_weights_q=top_p_weights(self.key_attention_weights_q,self.train_key_weight_top_p)
                    elif (not self.training) and (self.test_key_weight_top_p > 0):
                        self.key_attention_weights_q=top_p_weights(self.key_attention_weights_q,self.test_key_weight_top_p)
        ...
        if (self.distribution_v is not None) and (not self.is_decoder) and self.training:
            with torch.no_grad():
                for each_v,each_ids_w in zip(hidden_states,input_ids):
                    each_v=self.v(each_v.unsqueeze(0)).squeeze(0)
                    each_v=each_v[:len(each_ids_w)-(each_ids_w==0).long().sum()]
        ...
        if (self.previous_lora_weights_v is not None) and (not self.is_decoder) and(hidden_states.shape[1]>1):
            if past_key_attention_weights_v is not None:
                self.key_attention_weights_v=past_key_attention_weights_v
            else:
                with torch.no_grad():
                    key_v=None
                    for each_v,each_ids_w in zip(self.v(hidden_states),input_ids):
                        each_v=each_v[:len(each_ids_w)-(each_ids_w==0).long().sum()]
                        each_v=torch.mean(each_v,dim=0)
                        if key_v is None:
                    ...
                    self.key_attention_weights_v=calculate_distances(key_v,[self.distribution_v]+self.previous_lora_distribution_v,self.distances_way,self.distances_temperature)

                    if (self.training) and (self.train_key_weight_top > 0):
                        self.key_attention_weights_v=top_k_weights(self.key_attention_weights_v,self.train_key_weight_top)
                    elif (not self.training) and (self.test_key_weight_top > 0):
                        self.key_attention_weights_v=top_k_weights(self.key_attention_weights_v,self.test_key_weight_top)

                    if (self.training) and (self.train_key_weight_top_p > 0):
                        self.key_attention_weights_v=top_p_weights(self.key_attention_weights_v,self.train_key_weight_top_p)
                    elif (not self.training) and (self.test_key_weight_top_p > 0):
                        self.key_attention_weights_v=top_p_weights(self.key_attention_weights_v,self.test_key_weight_top_p)
...
class T5LayerSelfAttention(nn.Module):
    def __init__(self, config, prompt_config, has_relative_attention_bias=False):
        super().__init__()
        self.SelfAttention = T5Attention(config, prompt_config, has_relative_attention_bias=has_relative_attention_bias)
        self.layer_norm = T5LayerNorm(config.d_model, eps=config.layer_norm_epsilon)
        self.dropout = nn.Dropout(config.dropout_rate)
...
class T5LayerCrossAttention(nn.Module):
    def __init__(self, config, prompt_config):
        super().__init__()
        self.EncDecAttention = T5Attention(config, prompt_config, has_relative_attention_bias=False)
        self.layer_norm = T5LayerNorm(config.d_model, eps=config.layer_norm_epsilon)
        self.dropout = nn.Dropout(config.dropout_rate)
...
class T5Block(nn.Module):
    def __init__(self, config, prompt_config, has_relative_attention_bias=False):
        ...
        self.layer.append(T5LayerSelfAttention(config, prompt_config, has_relative_attention_bias=has_relative_attention_bias))
        if self.is_decoder:
            self.layer.append(T5LayerCrossAttention(config, prompt_config))
...
class T5Stack(T5PreTrainedModel):
    def __init__(self, config, embed_tokens=None, prompt_config=None):
        ...
        self.block = nn.ModuleList(
            [T5Block(config, prompt_config, has_relative_attention_bias=bool(i == 0)) for i in range(config.num_layers)]
        )
    ...
    def init_new_prompt(self, prompt_len):
        ...
        for i in range(prompt_len):
            with torch.no_grad():
                ...
                w = copy.deepcopy(self.embed_tokens.weight[j].detach().to(torch.float).cpu().numpy())
    ...
    def get_prompt(self):
        return self.prompt_encoder(self.prompt) + self.prompt
    ...
    def forward(
        ...
        for i, (layer_module, past_key_value) in enumerate(zip(self.block, past_key_values)):
            ...
            if self.successor is None:
                ...
                if i in self.successor:
                    past_key_attention_weights_v=None
                    past_key_attention_weights_q=None
                else:
                    past_key_attention_weights_q=self.block[i-1].layer[0].SelfAttention.key_attention_weights_q
                    past_key_attention_weights_v=self.block[i-1].layer[0].SelfAttention.key_attention_weights_v
...
class T5ForConditionalGeneration(T5PreTrainedModel):
    ...
    def memory_replay(self, input_ids, replay_labels):
        ...
        if replay_labels is not None:
            ...
            attn_scores = self.encoder.cal_attention(pre_prompt_key, inputs_embeds, return_logits=True)
            # print("attn_scores", torch.nn.functional.softmax(attn_scores.squeeze(2), 1))
            # print("replay_labels", replay_labels)
            # print('-'*20)
            # print(torch.nn.functional.softmax(attn_scores.squeeze(2), 1))
    ...
    def forward(
        ...
        key_attention_weights_q=self.encoder.block[-1].layer[0].SelfAttention.key_attention_weights_q
        key_attention_weights_v=self.encoder.block[-1].layer[0].SelfAttention.key_attention_weights_v

        # print(f'key_attention_weights_q: {key_attention_weights_q}')
        # print(f'key_attention_weights_v: {key_attention_weights_v}')


        for layers in self.decoder.block:
            layers.layer[0].SelfAttention.key_attention_weights_q=key_attention_weights_q
            layers.layer[0].SelfAttention.key_attention_weights_v=key_attention_weights_v
            layers.layer[1].EncDecAttention.key_attention_weights_q=key_attention_weights_q
            layers.layer[1].EncDecAttention.key_attention_weights_v=key_attention_weights_v
        ...
        if labels is not None:
            ...
            #         loss = loss - 0.5 * self.encoder.reduce_sim
