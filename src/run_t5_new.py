...
from t5_prompt_new import T5ForConditionalGeneration

from assets import task_config, lora_state_dict_A, lora_state_dict_B, lora_state_dict_distribution_T5
...
class ModelArguments:
    ...
    distances_way: str = field(
        default='L2',
        metadata={"help": "Path to load previous distribution"}
    )

    distances_temperature: float = field(
        default=1.0,
        metadata={"help": "Path to load previous distribution"}
    )
    train_key_weight_top: int = field(
        default=-1,
        metadata={"help": "Path to load previous distribution"}
    )

    test_key_weight_top: int = field(
        default=-1,
        metadata={"help": "Path to load previous distribution"}
    )

    train_key_weight_top_p: float = field(
        default=-1.0,
        metadata={"help": "Path to load previous distribution"}
    )

    test_key_weight_top_p: float = field(
        default=-1.0,
        metadata={"help": "Path to load previous distribution"}
    )
...
def main():
    ...
    if model_args.successor !='N':
        ...
        model_args.successor = [int(item) for item in lst]
    else:
        model_args.successor=None
    ...
    prompt_config = {
        ...
        'previous_lora_distribution_path': model_args.previous_lora_distribution_path,
        'task_id': cur_task_id,
        'run_single': model_args.run_single,
        'lora_r': model_args.lora_r,
        'lora_alpha': model_args.lora_alpha,
        'lora_dropout': model_args.lora_dropout,
        'distances_way':model_args.distances_way,
        'distances_temperature':model_args.distances_temperature,
        'train_key_weight_top':model_args.train_key_weight_top,
        'test_key_weight_top':model_args.test_key_weight_top,
        'train_key_weight_top_p':model_args.train_key_weight_top_p,
        'test_key_weight_top_p':model_args.test_key_weight_top_p,
    ...
    if model_args.previous_lora_distribution_path:
        ...
        for i, path in enumerate(previous_lora_distribution_list):
            ...
            for j in range(config.num_hidden_layers):
                old_distribution_q=distribution[f'encoder.block.{j}.layer.0.task.{cur_task_id-i-1}.q']
                old_distribution_v=distribution[f'encoder.block.{j}.layer.0.task.{cur_task_id-i-1}.v']

                old_distribution_q.mean=old_distribution_q.mean.to('cuda')
                old_distribution_q.var=old_distribution_q.var.to('cuda')

                old_distribution_v.mean=old_distribution_v.mean.to('cuda')
                old_distribution_v.var=old_distribution_v.var.to('cuda')

                model.encoder.block[j].layer[0].SelfAttention.previous_lora_distribution_q[i]=copy.deepcopy(old_distribution_q)
                model.encoder.block[j].layer[0].SelfAttention.previous_lora_distribution_v[i]=copy.deepcopy(old_distribution_v)
    ...
    if not training_args.do_train:
        ...
        for j in range(config.num_layers):
            model.encoder.block[j].layer[0].SelfAttention.lora_q.lora_A.data.copy_(
                lora_A[f"encoder.block.{j}.layer.0.SelfAttention.lora_q.lora_A"]
            )
            model.encoder.block[j].layer[0].SelfAttention.lora_q.lora_B.data.copy_(
                lora_B[f"encoder.block.{j}.layer.0.SelfAttention.lora_q.lora_B"]
            )
            model.encoder.block[j].layer[0].SelfAttention.lora_v.lora_A.data.copy_(
                lora_A[f"encoder.block.{j}.layer.0.SelfAttention.lora_v.lora_A"]
            )
            model.encoder.block[j].layer[0].SelfAttention.lora_v.lora_B.data.copy_(
                lora_B[f"encoder.block.{j}.layer.0.SelfAttention.lora_v.lora_B"]
            )
        
        ## Decoder Layaer
        for j in range(config.num_layers):
            model.decoder.block[j].layer[0].SelfAttention.lora_q.lora_A.data.copy_(
                lora_A[f"decoder.block.{j}.layer.0.SelfAttention.lora_q.lora_A"]
            )
            model.decoder.block[j].layer[0].SelfAttention.lora_q.lora_B.data.copy_(
                lora_B[f"decoder.block.{j}.layer.0.SelfAttention.lora_q.lora_B"]
            )
            model.decoder.block[j].layer[0].SelfAttention.lora_v.lora_A.data.copy_(
                lora_A[f"decoder.block.{j}.layer.0.SelfAttention.lora_v.lora_A"]
            )
            model.decoder.block[j].layer[0].SelfAttention.lora_v.lora_B.data.copy_(
                lora_B[f"decoder.block.{j}.layer.0.SelfAttention.lora_v.lora_B"]
            )
            model.decoder.block[j].layer[1].EncDecAttention.lora_q.lora_A.data.copy_(
                lora_A[f"decoder.block.{j}.layer.1.EncDecAttention.lora_q.lora_A"]
            )
            model.decoder.block[j].layer[1].EncDecAttention.lora_q.lora_B.data.copy_(
                lora_B[f"decoder.block.{j}.layer.1.EncDecAttention.lora_q.lora_B"]
            )
            model.decoder.block[j].layer[1].EncDecAttention.lora_v.lora_A.data.copy_(
                lora_A[f"decoder.block.{j}.layer.1.EncDecAttention.lora_v.lora_A"]
            )
            model.decoder.block[j].layer[1].EncDecAttention.lora_v.lora_B.data.copy_(
                lora_B[f"decoder.block.{j}.layer.1.EncDecAttention.lora_v.lora_B"]
            )
        ...
        for j in range(config.num_hidden_layers):
            old_distribution_q=distribution[f'encoder.block.{j}.layer.0.task.{cur_task_id}.q']
            old_distribution_v=distribution[f'encoder.block.{j}.layer.0.task.{cur_task_id}.v']
            ...
            model.encoder.block[j].layer[0].SelfAttention.distribution_q=copy.deepcopy(old_distribution_q)
            model.encoder.block[j].layer[0].SelfAttention.distribution_v=copy.deepcopy(old_distribution_v)
    ...
    if training_args.do_train:
        ...
        k=lora_state_dict_distribution_T5(model,cur_task_id)
        with open(os.path.join(save_path, f'lora_distribution.pkl'), 'wb') as file:
            pickle.dump(k, file)
