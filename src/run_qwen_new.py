...
def main():
    ...
    if model_args.previous_lora_distribution_path:
        ...
        for i, path in enumerate(previous_lora_distribution_list):
            ...
            ## Loading LoRA distributions for LLaMA-2
    ...
    if not training_args.do_train:
        ...
        for j in range(config.num_hidden_layers):
            model.model.layers[j].self_attn.lora_q.lora_A.data.copy_(
                lora_A[f"model.layers.{j}.self_attn.lora_q.lora_A"]
            )
            model.model.layers[j].self_attn.lora_q.lora_B.data.copy_(
                lora_B[f"model.layers.{j}.self_attn.lora_q.lora_B"]
            )
            model.model.layers[j].self_attn.lora_v.lora_A.data.copy_(
                lora_A[f"model.layers.{j}.self_attn.lora_v.lora_A"]
            )
            model.model.layers[j].self_attn.lora_v.lora_B.data.copy_(
                lora_B[f"model.layers.{j}.self_attn.lora_v.lora_B"]
            )
    ...
    if training_args.do_train:
        ...
        if is_main_process:
            ...
            k=lora_state_dict_distribution(model,cur_task_id)
